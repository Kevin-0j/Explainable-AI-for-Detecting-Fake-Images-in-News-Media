...
class ApiService {
  ...
  async detectDeepfake(file: File, onProgress?: UploadProgressCallback): Promise<DetectionResult> {
    ...
    return this.uploadDetection(formData, onProgress);
  }
  ...
  async detectFromUrl(imageUrl: string): Promise<DetectionResult> {
    ...
    return this.uploadDetection(formData);
  }
  ...
  async fetchLimeVisualization(resultId: string): Promise<string | null> {
    const data = await this.request<{ lime_visualization?: string }>(
      `/explain/lime/${resultId}`,
      {
        method: 'GET',
        headers: this.getAuthHeaders(),
      },
    ...
    return data.lime_visualization ?? null;
  }

  private uploadDetection(formData: FormData, onProgress?: UploadProgressCallback): Promise<DetectionResult> {
