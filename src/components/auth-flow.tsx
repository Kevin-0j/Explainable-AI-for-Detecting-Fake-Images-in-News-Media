...
export function AuthFlow({ onAuthSuccess, onBack }: AuthFlowProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (step === "login") {
        ...
        // Handle email verification code (for now, just proceed to login)
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // IMPORTANT: Users must complete Google OAuth setup at https://supabase.com/docs/guides/auth/social-login/auth-google
      ...
      if (errorMessage.includes('provider') || errorMessage.includes('enabled')) {
        toast.error(
          "Google sign-in is not configured yet. Please complete setup at: https://supabase.com/docs/guides/auth/social-login/auth-google",
          { duration: 8000 }
        );
      } else {
        toast.error(errorMessage);
        ...
        {step === "signup" && (
          <Card className="bg-card border-border">
            <CardHeader className="space-y-1">
              ...
              <CardDescription className="text-center text-muted-foreground">
                Join NewsCode to start verifying images
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Error Alert */}
